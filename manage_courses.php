...
if(isset($_POST['add_course'])){
    ...
    // Duplicate check
    $check = mysqli_query($conn, "SELECT id FROM courses WHERE course_name = '$c_name' OR course_code = '$c_code'");
    if(mysqli_num_rows($check) > 0){
        $msg = "<div class='alert alert-danger'>⚠️ Is naam ya code ka course pehle se maujood hai!</div>";
    } else {
        $ins = mysqli_query($conn, "INSERT INTO courses (course_name, course_code, duration, fees) VALUES ('$c_name', '$c_code', '$duration', '$fees')");
        if($ins){
            $msg = "<div class='alert alert-success'>✅ Course Safaltapurvak Add ho gaya!</div>";
        } else {
            $msg = "<div class='alert alert-danger'>❌ Error! Course add nahi ho saka.</div>";
        }
    }
}
...
if(isset($_GET['msg']) && $_GET['msg'] == 'deleted'){
    $msg = "<div class='alert alert-danger'>🗑️ Course successfully delete kar diya gaya!</div>";
}
?>
...
<head>
    ...
    <style>
        ...
        * { 
        ...
        .alert-success { background: rgba(46,204,113,0.15); color: #2ecc71; }
        ...
        .course-grid { display: grid; grid-template-columns: 1fr 1.6fr; gap: 30px; align-items: start; }

        .form-card { background: var(--card-bg); padding: 30px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.05); box-shadow: 0 10px 25px rgba(0,0,0,0.2); }
        ...
        .btn-submit { background: var(--primary); color: white; border: none; padding: 14px; border-radius: 10px; cursor: pointer; width: 100%; font-weight: 600; font-size: 15px; transition: 0.3s; }
        ...
        .code-badge { background: rgba(59, 130, 246, 0.15); color: #3b82f6; padding: 5px 12px; border-radius: 8px; font-size: 12px; font-weight: 700; display: inline-block; }
        ...
        /* 🖥️ DESKTOP VIEW (Screen width >= 992px) - UNTOUCHED & SHANDAR */
        ...
        /* 📱 MOBILE VIEW - PhonePe Native App Style with LARGER FONTS */
        @media (max-width: 991px) {
            ...
            /* Card & Form Updates with Larger Readable Font */
            .form-card {
            ...
            .code-badge { font-size: 12px !important; padding: 6px 10px !important; }
...
<!-- Mobile Fixed Top Bar (PhonePe Style) -->
...
<div class="sidebar" id="sidebar">
    ...
    <div style="padding: 0 10px;">
        ...
        <a href="manage_courses.php" class="active-nav"><i class="fa fa-graduation-cap"></i> <span>Courses</span></a>
        <a href="admin_upload_video.php"><i class="fa fa-video"></i> <span>Videos</span></a>
        <a href="admin_ebook.php"><i class="fa fa-book"></i> <span>E-Books</span></a>
...
<div class="main">
    ...
    <div class="course-grid">
        <!-- Add Course Form -->
        <div class="form-card">
            <h3 style="font-size: 18px; margin-bottom: 20px;"><i class="fas fa-plus-circle" style="color: var(--primary); margin-right: 8px;"></i> Add New Course</h3>
            <form action="" method="POST">
                <label>Course Full Name</label>
                <input type="text" name="course_name" placeholder="e.g. Diploma in Computer Application" required>
                
                <label>Course Short Code</label>
                <input type="text" name="course_code" placeholder="e.g. DCA, ADCA, TALLY" required>

                <label>Duration</label>
                <input type="text" name="duration" placeholder="e.g. 6 Months / 1 Year" required>

                <label>Total Fees (₹)</label>
                <input type="number" step="0.01" name="fees" placeholder="e.g. 4500" required>
                
                <button type="submit" name="add_course" class="btn-submit">
                    <i class="fas fa-save" style="margin-right: 8px;"></i> Save Course
                </button>
            </form>
        </div>
        ...
        <div class="table-container">
            <h3 style="font-size: 18px; margin-bottom: 15px;"><i class="fas fa-list" style="margin-right: 10px; color: var(--primary);"></i> Active Courses</h3>
                        ...
                        if(mysqli_num_rows($res) > 0) {
                            while($row = mysqli_fetch_assoc($res)){
                                echo "<tr>
                                        ...
                                        <td style='color: #94a3b8;'>{$row['duration']}</td>
                                        <td style='color: #2ecc71; font-weight:700;'>₹".number_format($row['fees'], 2)."</td>
                                        <td style='text-align: center;'>
                                            <a href='?delete_id={$row['id']}' class='btn-delete' onclick='return confirm(\"Kya aap is course ko delete karna chahte hain?\")'>
                            ...
                            echo "<tr><td colspan='5' style='text-align:center; padding: 30px; color: #64748b;'>Abhi tak koi course add nahi hua hai.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
...
<!-- PhonePe Mobile App Style Bottom Navigation -->
<div class="mobile-bottom-nav">
    ...
    <a href="manage_courses.php" class="phonepe-nav-item active">
...
<script>
    // Smooth Sidebar Drawer Toggle without Page Reload
